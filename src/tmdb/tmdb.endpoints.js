...
const tmdbEndpoints = {
  ...
  mediaTrending: ({ mediaType, timeWindow, page }) =>
    tmdbConfig.getUrl(`trending/${mediaType}/${timeWindow}`, { page }),
  discoverGenres: ({ mediaType, with_genres, page }) =>
    tmdbConfig.getUrl(`discover/${mediaType}`, { with_genres, page }),
  mediaDetail: ({ mediaType, mediaId, append_to_response }) =>
    tmdbConfig.getUrl(`${mediaType}/${mediaId}`, { append_to_response }),
  mediaSeasonDetail: ({ series_id, season_number }) => tmdbConfig.getUrl(`tv/${series_id}/season/${season_number}`),
